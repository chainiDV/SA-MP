
#include "PacketCipher.h"

#include <string.h>

unsigned short encrypt_descript_key;

unsigned char lookup_table[256] = {
#ifdef SAMPSRV
	
#else
	
#endif
};

unsigned char GetChecksum(unsigned char* buf, int len)
{
	unsigned char sum = 0;
	int i = 0;
	while (i != len)
	{
		sum ^= buf[i++] & 0xAA;
	}
	return sum;
}

void Transform(unsigned char lower, unsigned char upper, unsigned char* buf, int len)
{
	int switcher = 0;
	int i = 0;
	while (i != len)
	{
#ifndef SAMPSRV
		buf[i] = lookup_table[buf[i]];
#endif
		if (switcher)
		{
			buf[i] ^= upper;
			switcher--;
		}
		else
		{
			buf[i] ^= lower;
			switcher = 1;
		}
#ifdef SAMPSRV
		buf[i] = lookup_table[buf[i]];
#endif
		i++;
	}
}

#ifdef SAMPSRV

bool DecryptPacket(unsigned char* dest, unsigned char* data, int* len)
{
	unsigned char key = encrypt_descript_key;
	unsigned char sum = data[0];
	*len = *len - 1;

	memcpy(dest, data + 1, *len);
	Transform(0, key, dest, *len);
	return GetChecksum(dest, *len) == sum;
}

#else

void EncryptPacket(unsigned char* dest, unsigned char* data, int* len)
{
	unsigned char key = encrypt_descript_key;

	dest[0] = GetChecksum(data, *len);
	memcpy(dest + 1, data, *len);
	Transform(0, key, dest + 1, *len);
	*len = *len + 1;
}

#endif

void SetPacketCipherKey(int key)
{
	encrypt_descript_key = key ^ 0xCCCC;
}
