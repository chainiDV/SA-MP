...
{
	LaunchMod(GetAppPath().GetString(), "-d");
}
...
{
	LaunchMod(GetAppPath().GetString(), "-c -h 127.0.0.1 -p 7777 -n Player");
}
...
void LaunchMod(LPCSTR lpPath, LPCSTR lpParams)
{
	...
	BOOL bResult = CreateProcessWithDllA(szGtaExe,
		(LPSTR)lpParams,
		NULL,
		NULL,
		FALSE,
	...
	Proc_CreateProcessA FuncAdress)
{
	...
	if (FuncAdress(lpApplicationName,
		lpCommandLine,
		lpProcessAttributes,
		lpThreadAttributes,
		bInheritHandles,
